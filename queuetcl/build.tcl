...
proc _test {{config {}}} {
    ...
    # Then run the tests...
    set log [open LOG.queuetcl w]

    cd $::topdir/tests/queuetcl
...
proc _doc {{dst {../embedded}}} {
    ...
    file delete -force $dst/man/files/queuetcl
    file delete -force $dst/www/doc/files/queuetcl

    file mkdir $dst/man/files/queuetcl
    file mkdir $dst/www/doc/files/queuetcl

    puts "Generating man pages..."
    exec 2>@ stderr >@ stdout dtplite -ext n -o $dst/man nroff queuetcl
    puts "Generating 1st html..."
    exec 2>@ stderr >@ stdout dtplite -merge -o $dst/www html queuetcl
...
proc _figures {} {
    cd $::topdir/doc/queuetcl/figures
